...
while getopts "p:f:r:d:" 'OPTKEY'; do
    ...
    if [ "$file_of_accessions" = "" ]
        then
            ...
            ls -d /scratch/$account/$user/"$root_project"/"$project"/contigs/final_contigs/*.fa > /scratch/$account/$user/"$root_project"/"$project"/contigs/final_contigs/file_of_accessions_for_blastx_RVDB
            export file_of_accessions="/scratch/$account/$user/$root_project/$project/contigs/final_contigs/file_of_accessions_for_blastx_RVDB"
        else    
...
if [ "$db" = "" ]
    then
        echo "No database specified. Use -d option to specify the database, e.g, -d /scratch/$account/$user/VELAB/Databases/Blast/RVDB/U-RVDBv22.0-prot-exo_curated.dmnd"
        exit 1
fi

sbatch --array $jPhrase \
    --output "/scratch/$account/$user/$root_project/$project/logs/blastxRVDB_%A_%a_$project_$(date '+%Y%m%d')_stout.txt" \
    --error="/scratch/$account/$user/$root_project/$project/logs/blastxRVDB_%A_%a_$project_$(date '+%Y%m%d')_stderr.txt" \
    --export="project=$project,file_of_accessions=$file_of_accessions,root_project=$root_project,singularity_image=$singularity_image,account=$account,user=$user,db=$db,input=$input" \
    --time "12:00:00" \
    --account="$account" \
    /scratch/$account/$user/"$root_project"/"$project"/scripts/JCOM_pipeline_blastxRVDB.slurm