...
while getopts "p:f:r:d:" 'OPTKEY'; do
    ...
    if [ "$file_of_accessions" = "" ]
        then
            ...
            ls -d /scratch/"$root_project"/"$project"/contigs/final_contigs/*.fa > /scratch/"$root_project"/"$project"/contigs/final_contigs/file_of_accessions_for_blastxNR
            export file_of_accessions="/scratch/$root_project/$project/contigs/final_contigs/file_of_accessions_for_blastxNR"
        else    
...
sbatch --array $jPhrase \
    --output "/group/$root_project/$project/logs/blastnr_$SLURM_ARRAY_TASK_ID_$project_$queue_$db_$(date '+%Y%m%d')_stout.txt" \
    --error="/group/$root_project/$project/logs/blastnr_$SLURM_ARRAY_TASK_ID_$project_$queue_$db_$(date '+%Y%m%d')_stderr.txt" \
    ...
    /group/"$root_project"/"$project"/scripts/JCOM_pipeline_blastnr.pbs